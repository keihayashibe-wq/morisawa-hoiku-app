...
interface AbsenceReport {
  id: string; childId: string; date: string; type: string; reason: string | null;
  arrivalTime: string | null; pickupTime: string | null; confirmed: boolean;
...
const TYPE_COLORS: Record<string, string> = {
  ...
  EARLY_PICKUP: "bg-orange-100 text-orange-700",
};
...
export default function AbsencePage() {
  ...
  const [form, setForm] = useState({ childId: "", date: getTodayString(), type: "ABSENT", reason: "", arrivalTime: "", pickupTime: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch("/api/absence", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    setShowForm(false);
    load();
  };

  const handleConfirm = async (id: string) => {
    await fetch("/api/absence", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, confirmed: true }) });
    load();
  };
  ...
  return (
    <div className="container-app py-6">
      ...
      {user.role === "PARENT" && (
        <button onClick={() => setShowForm(true)} className="btn btn-primary mb-4 w-full sm:w-auto">欠席・遅刻を連絡する</button>
      )}

      {reports.length === 0 ? (
        <div className="card text-center text-gray-400 py-8">連絡はありません</div>
      ) : (
          ...
          {reports.map(r => (
            <div key={r.id} className="card">
              <div className="flex items-center justify-between mb-2">
                ...
                {r.confirmed ? (
                  <span className="badge bg-green-100 text-green-700">確認済</span>
                ) : user.role !== "PARENT" ? (
                  <button onClick={() => handleConfirm(r.id)} className="btn btn-primary text-xs !px-3 !py-1 !min-h-0">確認する</button>
                ) : (
                  <span className="badge bg-gray-100 text-gray-500">未確認</span>
                )}
              </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {Object.entries(TYPE_LABELS).map(([v, l]) => (
                    <button key={v} type="button" className={`tab flex-1 ${form.type === v ? "active" : ""}`} onClick={() => setForm({ ...form, type: v })}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button type="submit" className="btn btn-primary flex-1">送信</button>
