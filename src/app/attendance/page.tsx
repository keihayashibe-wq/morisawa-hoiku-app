    ...
    <div className="container-app py-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-sm text-gray-500">
          出席: {records.filter((r) => r.checkInTime).length} / {children.length}名
        </span>
      </div>
...
function ChildAttendanceCard({
  ...
  onAction: (childId: string, action: "checkin" | "checkout", pickupPerson?: string) => void;
}) {
    ...
    <div className="card">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400">{child.class?.name || "未配属"}</p>
        </div>

        <div className="flex items-center gap-2">
          {isCheckedIn && (
            <span className="badge bg-green-100 text-green-700">
              登園 {record?.checkInTime}
            </span>
          )}
          {isCheckedOut && (
            <span className="badge bg-blue-100 text-blue-700">
              降園 {record?.checkOutTime}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-3">
        {!isCheckedIn ? (
          <button onClick={() => onAction(child.id, "checkin")} className="btn btn-primary flex-1 text-sm">
            🏫 登園する
          </button>
        ) : !isCheckedOut ? (
          <>
            {!showPickup ? (
              <button onClick={() => setShowPickup(true)} className="btn btn-secondary flex-1 text-sm">
                🏠 降園する
              </button>
            ) : (
              <div className="flex gap-2 flex-1">
                ...
                />
                <button
                  onClick={() => { onAction(child.id, "checkout", pickupPerson); setShowPickup(false); }}
          ...
          <span className="text-sm text-gray-400 py-2">
            {record?.pickupPerson ? `お迎え: ${record.pickupPerson}` : "降園済み"}
          </span>
        )}
      </div>
    </div>
