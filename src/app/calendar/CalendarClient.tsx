...
interface CalendarEvent { id: string; title: string; description: string | null; date: string; startTime: string | null; endTime: string | null; location: string | null; category: string; allDay: boolean; }
const CATEGORY_ICONS: Record<string, string> = { EVENT: "🎉", HOLIDAY: "🌸", HEALTH_CHECK: "🏥" };

interface Props { user: { role: string }; initialEvents: CalendarEvent[]; initialYear: number; initialMonth: number; }

export default function CalendarClient({ user, initialEvents, initialYear, initialMonth }: Props) {
  ...
  const loadEvents = (ms: string) => fetch(`/api/calendar?month=${ms}`).then(r => r.json()).then(setEvents);
  const prevMonth = () => { const m = month === 1 ? 12 : month - 1; const y = month === 1 ? year - 1 : year; setMonth(m); setYear(y); loadEvents(`${y}-${String(m).padStart(2, "0")}`); };
  const nextMonth = () => { const m = month === 12 ? 1 : month + 1; const y = month === 12 ? year + 1 : year; setMonth(m); setYear(y); loadEvents(`${y}-${String(m).padStart(2, "0")}`); };

  const daysInMonth = new Date(year, month, 0).getDate();
  const firstDayOfWeek = new Date(year, month - 1, 1).getDay();
  const canEdit = user.role !== "PARENT";

  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); await fetch("/api/calendar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); setShowForm(false); loadEvents(monthStr); };
    ...
    <div className="container-app py-6">
      ...
      <div className="card mb-4 overflow-x-auto">
        <div className="grid grid-cols-7 gap-0 min-w-[320px]">
          {["日","月","火","水","木","金","土"].map((d,i) => <div key={d} className={`text-center text-xs font-semibold py-2 ${i===0?"text-red-500":i===6?"text-blue-500":"text-gray-500"}`}>{d}</div>)}
          ...
          {Array.from({length:daysInMonth}).map((_,i) => {
            ...
            return <div key={day} className={`min-h-[48px] p-1 border-t border-gray-100 ${isToday?"bg-[var(--color-primary-light)]":""}`}>
              <span className={`text-xs font-medium ${dow===0?"text-red-500":dow===6?"text-blue-500":""}`}>{day}</span>
              {dayEvents.map(ev => <div key={ev.id} className="text-[10px] bg-[var(--color-primary)] text-white rounded px-1 mt-0.5 truncate">{ev.title}</div>)}
            </div>;
          })}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {events.length === 0 ? <div className="card text-center text-gray-400 py-4">この月の行事はありません</div> :
          events.map(ev => <div key={ev.id} className="card !p-3">
            <div className="flex items-center gap-2"><span>{CATEGORY_ICONS[ev.category]||"📌"}</span><span className="font-semibold text-sm">{ev.title}</span><span className="text-xs text-gray-400 ml-auto">{ev.date.split("-")[2]}日</span></div>
      ...
      {showForm && (
        <div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" onClick={e => e.stopPropagation()}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div><label className="label">カテゴリ</label><select className="input" value={form.category} onChange={e => setForm({...form,category:e.target.value})}><option value="EVENT">行事</option><option value="HOLIDAY">休園日</option><option value="HEALTH_CHECK">健康診断</option></select></div>
