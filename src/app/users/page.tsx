...
export default function UsersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setForm({ email: "", password: "", name: "", role: "PARENT", phone: "" });
    load();
  };

  if (!user || user.role !== "ADMIN") return <div className="container-app py-6"><div className="card text-center text-gray-400 py-8">このページは管理者専用です</div></div>;
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {Object.entries(ROLE_LABELS).map(([v, l]) => (
                    <button key={v} type="button" className={`tab flex-1 ${form.role === v ? "active" : ""}`} onClick={() => setForm({ ...form, role: v })}>{l}</button>
                  ))}
                </div>
              </div>
