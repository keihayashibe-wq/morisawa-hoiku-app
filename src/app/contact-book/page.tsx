...
interface ContactEntry {
  ...
  homeMood: string | null;
  homeTemp: number | null;
  homeMeal: string | null;
  homeSleep: string | null;
  homeBowel: string | null;
  homeNotes: string | null;
  schoolMood: string | null;
  schoolTemp: number | null;
  schoolMeal: string | null;
  schoolSleep: string | null;
  schoolBowel: string | null;
  schoolActivity: string | null;
...
export default function ContactBookPage() {
  ...
  const [form, setForm] = useState({
    homeMood: "", homeTemp: "", homeMeal: "", homeSleep: "", homeBowel: "", homeNotes: "",
    schoolMood: "", schoolTemp: "", schoolMeal: "", schoolSleep: "", schoolBowel: "", schoolActivity: "", schoolNotes: "",
  });

  useEffect(() => {
    fetch("/api/children").then((r) => r.json()).then((data) => {
      setChildren(data);
  ...
  useEffect(() => {
    if (selectedChild) {
      fetch(`/api/contact-book?childId=${selectedChild}&date=${date}`)
        .then((r) => r.json())
        .then(setEntries);
    }
  }, [selectedChild, date]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await fetch("/api/contact-book", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ childId: selectedChild, date, ...form }),
    });
    setShowForm(false);
    const res = await fetch(`/api/contact-book?childId=${selectedChild}&date=${date}`);
  ...
  return (
    <div className="container-app py-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <select className="input flex-1" value={selectedChild} onChange={(e) => setSelectedChild(e.target.value)}>
          {children.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <input type="date" className="input sm:w-44" value={date} onChange={(e) => setDate(e.target.value)} />
      </div>

      <button onClick={() => setShowForm(true)} className="btn btn-primary mb-4 w-full sm:w-auto">
        {isParent ? "家庭からの連絡を記入" : "園からの連絡を記入"}
      </button>

      {/* Entries */}
      {entries.length === 0 ? (
        <div className="card text-center text-gray-400 py-8">この日の連絡帳はまだありません</div>
      ) : (
        entries.map((entry) => (
          <div key={entry.id} className="card mb-3">
            <div className="flex justify-between items-center mb-3">
              <span className="font-semibold">{entry.child.name}</span>
              <span className="text-xs text-gray-400">{entry.author.name}（{entry.author.role === "PARENT" ? "保護者" : "保育士"}）</span>
            </div>

            {(entry.homeMood || entry.homeTemp || entry.homeNotes) && (
              <div className="bg-orange-50 rounded-xl p-3 mb-2">
                <h3 className="text-sm font-semibold text-orange-700 mb-2">🏠 家庭から</h3>
            ...
            {(entry.schoolMood || entry.schoolTemp || entry.schoolNotes) && (
              <div className="bg-green-50 rounded-xl p-3">
                <h3 className="text-sm font-semibold text-green-700 mb-2">🏫 園から</h3>
                <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {isParent ? (
                <>
                  <div>
                    <label className="label">機嫌</label>
                    <div className="flex gap-2">
                      {MOOD_OPTIONS.map((m) => (
                        <button key={m.value} type="button"
                          className={`tab flex-1 ${form.homeMood === m.value ? "active" : ""}`}
                          onClick={() => setForm({ ...form, homeMood: m.value })}
                        >{m.label}</button>
                      ))}
                    </div>
                  </div>
                ...
                <>
                  <div>
                    ...
                    <div className="flex gap-2">
                      {MOOD_OPTIONS.map((m) => (
                        <button key={m.value} type="button"
                          className={`tab flex-1 ${form.schoolMood === m.value ? "active" : ""}`}
                          onClick={() => setForm({ ...form, schoolMood: m.value })}
                        >{m.label}</button>
                      ))}
                    </div>
                  </div>
